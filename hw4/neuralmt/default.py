...
class AttentionModule(nn.Module):
    def __init__(self, attention_dim):
        """
        You shouldn't delete/change any of the following defs, they are
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        seq1, batch1, _ = decoder_hidden.shape
        scores = torch.Tensor([seq * [batch * [1]]]).permute(2, 1, 0) + torch.Tensor([seq1 * [batch1 * [1]]]).permute(2, 1, 0)
        alpha = torch.nn.functional.softmax((dim * torch.tanh(scores)), dim=0).reshape(1,1,dim)
        return alpha
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        context = (torch.sum((encoder_out / seq) * alpha[0], dim=0)).reshape(1, 1, dim)
