...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, char_dim, second_rnn_dim):
        torch.manual_seed(1)
        super(LSTMTaggerModel, self).__init__()
        self.hidden_dim = hidden_dim
        ##################################################
        self.second_rnn_dim = second_rnn_dim
        self.second_rnn = secondRNN(char_dim, second_rnn_dim)
        ######################################################
        ...
        self.lstm = nn.LSTM(embedding_dim + second_rnn_dim, hidden_dim, bidirectional=False)
    ...
    def forward(self, sentence, input_vectors):
        ...
        second_rnn = self.second_rnn(input_vectors.view(len(sentence), 1, -1), torch.zeros(len(sentence), 1, self.second_rnn_dim))
        char_feat = torch.cat((embeds, second_rnn.squeeze(1)), 1)
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), char_dim = 300, second_rnn_dim = 64)
