...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, char_dim):
        torch.manual_seed(1)
    ...
    def forward(self, sentence, input_vectors):
        embeds = self.word_embeddings(sentence)
        char_feat = torch.cat((embeds, input_vectors), 1)
        lstm_out, _ = self.lstm(char_feat.view(len(sentence), 1, -1))
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), char_dim = 300)
    ...
    def argmax(self, seq):
        ...
        with torch.no_grad():
            ...
            input_vectors = char_lvl_representation(seq)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                input_vectors = char_lvl_representation(sentence)
