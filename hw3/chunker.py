# -*- coding: utf-8 -*-
"""chunker
DEPRECATED -- FIRST SOLUTION -- CURRENT SOLUTION: option2_rnn.py
"""
...
def char_level_repr(seq, width = 100):
  input_vectors = []
  for element in seq:
    vec1 = torch.zeros(width)
    vec2 = torch.zeros(width)
    vec3 = torch.zeros(width)
    # now assigning values to the vectors
    if element is not '[UNK]':
      vec1[string.printable.find(element[0])] = 1
      vec3[string.printable.find(element[-1])] = 1
      for uniqueWord in (list(set(element[1:-1]))):
        vec3[string.printable.find(uniqueWord)] = element.count(uniqueWord)
    # append to input_vectors
    input_vectors.append(torch.cat(vec1, vec2, vec3), 0)
    torchStack = torch.stack(input_vectors)
  return torchStack

class LSTMTaggerModel(nn.Module):

    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, dim_input_vec):
        torch.manual_seed(1)
        ...
        self.lstm = nn.LSTM(embedding_dim + dim_input_vec, hidden_dim, bidirectional=False)
    ...
    def forward(self, sentence):
        ...
        features = torch.cat((embeds, input_vectors), 1)
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), dim_input_vec = 300)
    ...
    def argmax(self, seq):
        ...
        with torch.no_grad():
            ...
            input_vectors = char_level_repr(seq)
            tag_scores = self.model(inputs, input_vectors)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                input_vectors = char_level_repr(sentence)
                targets = prepare_sequence(tags, self.tag_to_ix, self.unk)

                # Step 3. Run our forward pass.
                tag_scores = self.model(sentence_in, input_vectors)
